...
class AttentionScorer:
    ...
    def get_PERCLOS(self, ear_score, current_time=None):
        ...
        :param current_time: float or None
            Current time in seconds. If None, time.time() will be used
        :return:
        ...
        # Initialize prev_time if it's the first call
        if self.prev_time == 0:
            self.prev_time = current_time

        delta = current_time - self.prev_time  # set delta timer
        ...
        # PERCLOS should be the ratio of closure time to the elapsed time or the full period, whichever is smaller
        elapsed_time = min(delta, self.perclos_time_period)
        if elapsed_time > 0:
            perclos_score = closure_time / elapsed_time
        else:
            perclos_score = 0.0
        ...
        if self.verbose:
            print(
                f"Delta Time: {delta:.2f}s | Closure Time: {closure_time:.2f}s/{self.perclos_time_period}s | PERCLOS: {round(perclos_score, 3)} | Eye Closure Counter: {self.eye_closure_counter}")
